...
// 미션
// 1. 아래 useEffect가 필요한 이유 숙지
// 2. 유튜브 제목, 본문, 유튜브 썸네일(Pic 컴포넌트 활용), 날짜까지 출력
...
/*
	...
	- then구문안에서 서버에서 가져온 데이터를 변수에 옮겨담았을때 해당 지역변수에 들어가 있는 값을 then구문 밖에서 호출 할 방법 없음
	- 설사 데이터를 전역변수, querySelector를 이용해서 꺼내온다고 하더라도
	- 특정 state가 변경되면 컴포넌트 함수자체가 재호출되는 특성상 fetching 연산이 매법 재실행됨

	위와 같은 문제점 해결 방법
	- 의존성배열이 비어있는 useEffect의 콜백함수 안쪽에서 data fetching 처리 후 state에 담아줌
	- 설사 다른 state같이 변경되서 컴포넌트가 자주 재호출되더라도 연산 처리가 많이 되는fetching 함수의 호출을 컴포넌트읨 ㅏ운트 시점으로 제한
	- 컴포넌트가 언마운트 되지 않는한 data fetching과 그 결과값을 state에 담아주는 로직은 재연산, 재호출 되지 않음
